...
import { TextInput, Button, Text } from 'react-native-paper'; // Import Portal, Modal, and IconButton
import {
    collection,
    addDoc,
    Timestamp,
...
const HomeScreen = ({ navigation }) => {
    ...
    const handleAddEntry = async () => {
        ...
        if (cigaretteCount && cigarettePrice && !isNaN(parseFloat(cigarettePrice))) {
          ...
          if (!entries.length) {
            // Handle the case where there are no existing entries
            ...
            console.log(singleCig);

            if(!singleCig) {
                setIsLoading(false)
                return alert('No single entries!')

            } 

            const newEntry = {
              count: singleCig ? singleCig.count : cigaretteCount,
              price: parseFloat(singleCig ? singleCig.price : cigarettePrice),
              timestamp: Timestamp.now(),
            };
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.container}>
                ...
                >
                    {isLoading ? 'Adding...' : 'Add'} {/* Display "Adding..." while loading */}
                </Button>
            </View>
        </SafeAreaView>
    );
